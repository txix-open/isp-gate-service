### v5.11.4
* добавлена возможность через переменную окружения `adminServiceName` переопределить название сервиса `msp-admin-service`
### v5.11.3
* добавлена настройка для преопределения content-type в ответе с `application/json` на `application/json; charset=utf-8` для конкретных приложений
### v5.11.2
* обновлены зависимости
### v5.11.1
* Обновлен isp-kit
### v5.11.0
* Добавлена поддержка нескольких `tokenProvider` для `customAuth.userAuthSettings`: используется первый провайдер, вернувший токен
### v5.10.2
...
### v5.9.0
* Теперь в зависимоси от `StatusCode` выводится соотвествующий уровень логирования: 400 -> warn, 500 -> error
* Исправлен порядок мидлварей, для корректного проброса `requestId`
### v5.8.1
* Обновлены зависимости
### v5.8.0
...
### v5.6.1
* Добавлен проброс заголовка `x-application-name`
### v5.6.0
* Добавлена поддержка метрик для http запросов
### v5.5.1
* повышен приоритет `x-application-token` над basic-auth, при поиске в запросе токена приложения
### v5.5.0
* Добавлена поддержка basic-auth, где `username` - название приложения, `password` - токен приложения
### v5.4.1
* Оптимизация очистки тела запроса от экранирования unicode символов для корректного логирования
### v5.4.0
* Добавлена настройка `forwardReqIdClientSettings` для разрешения прокидывания `requestId` определённым приложениям по их идентификаторам (`requestId` клиента прокидывается если в `forwardReqIdClientSettings` для приложения включено прокидывание или включен глобальный флаг `enableClientRequestIdForwarding`)
